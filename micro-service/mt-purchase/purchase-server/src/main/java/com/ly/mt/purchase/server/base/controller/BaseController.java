package com.ly.mt.purchase.server.base.controller;
...
@RestController
@RequestMapping("/purchase/server")
public class BaseController {
    ...
    public JSONObject requestDistribute(@RequestBody(required = false) JSONObject jsonObject) {
        ...
        try {
            ...
            LOGGER.info("purchase-server:接口={},方法={},执行耗时={}", serviceName, functionName, String.valueOf(end - start));
            return jsonObject;
        } catch (Exception e) {
            LOGGER.error("purchase-server:接口={},方法={},调用异常:", serviceName, functionName, e);
            return JsonUtil.getErrorJson(RESULT_CODE_SYSTEM_ERROR);
        }
    }
}