package com.ly.mt.cabinet.c.code.service.impl;
...
@Service
public class GzgDynamicCodeServiceImpl extends BaseServiceImpl implements GzgDynamicCodeService {
    private final static Logger LOGGER = LoggerFactory.getLogger(GzgDynamicCodeServiceImpl.class);
    @Resource
    YmlConfig yml;
    ...
    public ResponseJson sendDynamicCode(String mobile, String id) {
        try {
            ...
            if (StringUtil.isNotEmpty(redisCode)) {
                return ResponseUtil.getResponseMsg(RESPONSE_CODE_ERROR, "60秒内不能重复获取!");
            }
            // 生成并发送验证码
